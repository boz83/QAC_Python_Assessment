...
def two(input):
  ...
  inputLower = inputLower.split("bert")
  if(len(inputLower) <= 2):
    return  ""
  else:
    return inputLower[1]
...
'''
I wanted to use regex as its a lot neater but the code i used didnt pass the rest even though
it yielded the same result

def two(string):
        regex = re.findall(r"(?i)bert(.*)bert", string)
        return(regex)
'''
	# <QUESTION 3>
